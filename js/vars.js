...
const message = (desc, el) => {
  desc === "desc"
    ? (el.textContent = dTWarn)
    : desc === "amount"
    ? (el.textContent = aTWarn)
    : null;
  el.style.color = "red";
  setTimeout(() => {
    desc === "desc"
      ? (el.textContent = dText)
      : desc === "amount"
...
const display = (state) => {
  ...
  iSum.textContent =
    i.length == 0 ? "No Income Yet" : `${i.length} Income Sources`;

  exTotl.textContent = x.total === 0 ? `$ 0.00` : `$ - ${fNum(x.total)}`;
  exSum.textContent =
    x.length == 0 ? "No Income Yet" : `${x.length} Expense Items`;

  balTotl.textContent = b.balance === 0 ? `$ 0.00` : `$ ${fNum(b.balance)}`;
  balPercnt.textContent =
    b.percent === 0 ? `No Saving Yet` : `${b.percent}% Savings`;

  bar.innerHTML = x.list
    .map(
      (li) => `
  ...
  if (x.list.length == 0) {
    ...
    expenseList.innerHTML = x.list
      .map(
        (ex) => `
    <li class='listExpenses' style="background-color: rgb(${ex.color.r}, ${
          ex.color.g
        }, ${ex.color.b})">
